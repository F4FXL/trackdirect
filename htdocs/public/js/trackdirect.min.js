...
100000).toFixed();days=days*sign;if(type=="lat")direction=days<0?"S":"N";if(type=="lon")direction=days<0?"W":"E";return(days*sign).toString().padStart(type=="lat"?2:3,"0")+"º "+minutes.toString().padStart(2,"0")+"' "+secounds.toString().padStart(2,"0")+"'' "+direction;};trackdirect.models.Map.prototype._getMaidenheadLocatorFromGpsDecimal=function(lat,lon,){if(lat<-90||lat>90||lon<-180||lon>180)return null;lon+=180;lat+=90;var A='ABCDEFGHIJKLMNOPQRSTUVWXYZ';var fieldLon=Math.floor(lon/20);var fieldLat=Math.floor(lat/10);var squareLon=Math.floor((lon%20)/2);var squareLat=Math.floor((lat%10)/1);var subsquareLon=Math.floor(((lon%2)/2)*24);var subsquareLat=Math.floor(((lat%1)/1)*24);return(A.charAt(fieldLon)+
A.charAt(fieldLat)+
squareLon.toString()+
squareLat.toString()+

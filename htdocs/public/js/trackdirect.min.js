var trackdirect={services:{},models:{},_time:null,_timetravel:null,_center:null,_zoom:null,_maptype:null,_mid:null,_rulers:[],_filterTimeoutId:null,_waitForFilterResponse:false,_doNotChangeLocationOnFilterResponse:false,_doNotChangeLocationOnFilterResponseTmp:false,_filters:{},_defaultLatitude:null,_defaultLongitude:null,_eventListeners:{},_eventListenersOnce:{},_coordinatesContainerElementId:null,_statusContainerElementId:"td-status-text",_mapElementId:null,_wsServerUrl:null,_map:null,_websocket:null,_mapCreated:false,_trackdirectInitDone:false,_coverageType:'movingonly',isMobile:false,coverageDataUrl:null,coveragePercentile:95,settings:{},init:function(wsServerUrl,mapElementId,options){this._initSettings();this._wsServerUrl=wsServerUrl;this._mapElementId=mapElementId;if($("#"+mapElementId).length<=0){console.log("ERROR: Specified map element missing");return;}
...
this._eventListeners[event].push(handler);}},setCenter:function(latitude,longitude,zoom){latitude=typeof latitude!=="undefined"?latitude:this._defaultLatitude;longitude=typeof longitude!=="undefined"?longitude:this._defaultLongitude;zoom=typeof zoom!=="undefined"?zoom:this._map.getZoom();if(this._map!==null){this._map.setCenter({lat:latitude,lng:longitude},zoom);}},setCenterLocator:function(locator){if(!this.isValidMaidenhead(locator))return false;let zoom=15;locator=locator.trim();const A='ABCDEFGHIJKLMNOPQRSTUVWXYZ';const a='abcdefghijklmnopqrstuvwxyz';let lon=-180,lat=-90;let sizeLon=20,sizeLat=10;if(locator.length>=2){lon+=A.indexOf(locator[0].toUpperCase())*20;lat+=A.indexOf(locator[1].toUpperCase())*10;zoom=4;}
if(locator.length>=4){lon+=parseInt(locator[2])*2;lat+=parseInt(locator[3])*1;sizeLon/=10;sizeLat/=10;zoom=6;}
if(locator.length>=6){lon+=A.indexOf(locator[4].toUpperCase())*(sizeLon/24);lat+=A.indexOf(locator[5].toUpperCase())*(sizeLat/24);sizeLon/=24;sizeLat/=24;zoom=10;}
if(locator.length>=8){lon+=parseInt(locator[6])*(sizeLon/10);lat+=parseInt(locator[7])*(sizeLat/10);sizeLon/=10;sizeLat/=10;zoom=13;}
if(locator.length>=10){lon+=a.indexOf(locator[8].toLowerCase())*(sizeLon/24);lat+=a.indexOf(locator[9].toLowerCase())*(sizeLat/24);sizeLon/=24;sizeLat/=24;zoom=16;}
lat+=sizeLat/2;lon+=sizeLat/2;if(isNaN(lat)||isNaN(lon))
return false;this.setCenter(lat,lon,zoom);return true;},isValidMaidenhead:function(locator){if(typeof locator!=="string")return false;locator=locator.trim();const n=locator.length;if(![2,4,6,8,10].includes(n))return false;const letter="[A-Za-z]";const digit="[0-9]";const letterAX="[A-Xa-x]";const regex={2:new RegExp(`^${letter}{2}$`),4:new RegExp(`^${letter}{2}${digit}{2}$`),6:new RegExp(`^${letter}{2}${digit}{2}${letter}{2}$`),8:new RegExp(`^${letter}{2}${digit}{2}${letter}{2}${digit}{2}$`),10:new RegExp(`^${letter}{2}${digit}{2}${letter}{2}${digit}{2}${letterAX}{2}$`)};return regex[n].test(locator);},setZoom:function(value){if(this._map!==null){this._map.setZoom(value);}},addRuler:function(){if(this._rulers.length>0){var ruler=this._rulers.pop();ruler.hide();}else{var ruler=new trackdirect.models.Ruler((this._map.getCurrentRadiusInKm()*1000)/2,this._map);this._rulers.push(ruler);}},isFilteredMode:function(){return this._map.state.isFilterMode;},stopFilterOnStationId:function(stationId){var currentFilterStationIds=this._map.state.getFilterStationIds();if(currentFilterStationIds.length==1&&currentFilterStationIds.indexOf(stationId)>=0){this.filterOnStationId([]);}else{if(this._map.state.getTrackStationId()==stationId){this.stopTrackStation();}
...
marker.show();marker.showLabel();this.setCenter(marker.packet.latitude,marker.packet.longitude,zoom);map.openMarkerInfoWindow(marker);marker.hide(5000,true);}},toggleStationCoverage:function(stationId,isUserInput){this._map.state.openInfoWindow?.UpdateCoverageLink("begin",isUserInput);var coveragePolygon=this._map.markerCollection.getStationCoverage(stationId);if(coveragePolygon!==null&&coveragePolygon.isRequestedToBeVisible()){coveragePolygon.hide();this._map.markerCollection.addStationCoverage(stationId,null);this._map.state.openInfoWindow?.UpdateCoverageLink("hidden",isUserInput);}
else{var packet=this._map.markerCollection.getStationLatestPacket(stationId);var center={lat:parseFloat(packet.latitude),lng:parseFloat(packet.longitude),};var coveragePolygon=new trackdirect.models.StationCoveragePolygon(center,this._map,true);coveragePolygon.showWhenDone();var me=this;coveragePolygon.addTdListener("visible",function(){if(!coveragePolygon.hasContent()){coveragePolygon.hide();me._map.state.openInfoWindow?.UpdateCoverageLink("not-enough-data",isUserInput);}else{me._map.state.openInfoWindow?.UpdateCoverageLink("shown",isUserInput);}},true);$.getJSON(this.coverageDataUrl+"?id="+stationId+"&coveragetype="+this._coverageType,function(data){if("station_id"in data&&"coverage"in data){coveragePolygon.setData(data["coverage"],me.coveragePercentile);var marker=me._map.markerCollection.getStationLatestMarker(stationId);if(marker.isVisible()){if(coveragePolygon.isRequestedToBeVisible()){me._map.markerCollection.addStationCoverage(stationId,coveragePolygon);coveragePolygon.show();}}}}).fail(function(){coveragePolygon.hide();this._map.markerCollection.addStationCoverage(stationId,null);me._map.state.openInfoWindow?.UpdateCoverageLink("load-failed",isUserInput);}).always(function(){});}
},setCoverageType:function(coverageType){if(this._coverageType!=coverageType){const ids=this._map.markerCollection.getStationCoverageIds();this._coverageType=coverageType;for(const stationId of ids){this.toggleStationCoverage(parseInt(stationId),false);this.toggleStationCoverage(parseInt(stationId),false);}}},setMapType:function(mapType){if(this._map!==null){this._map.setMapType(mapType);}},getMapType:function(){if(this._map!==null){return this._map.getMapType();}},setMapDefaultLocation:function(setDefaultZoom){this._map.setMapDefaultLocation(setDefaultZoom);},setMapLocationByGeoLocation:function(failCallBack,successCallBack,timeout){var me=this;if(navigator&&navigator.geolocation){navigator.geolocation.getCurrentPosition(function(position){var pos={lat:position.coords.latitude,lng:position.coords.longitude,};me._map.setCenter(pos,12);if(successCallBack!==null){successCallBack();}},function(error){if(failCallBack!==null){failCallBack(error.message);}},{enableHighAccuracy:false,timeout:timeout,maximumAge:5000,});}else{if(failCallBack!==null){failCallBack();}}},openStationInformationDialog:function(stationId){var packet=this._map.markerCollection.getStationLatestPacket(stationId);if(packet==null){packet={station_id:stationId,id:null};}
...
this._emitEventListeners("time-length-changed",time);this._updateMinZoomLevels();},getTimeLength:function(){return this._map.state.getTimeLength();},togglePHGCircles:function(){var state=this._map.state;if(state.showPHGCircles==0){state.showPHGCircles=1;}else if(state.showPHGCircles==1){state.showPHGCircles=2;}else{state.showPHGCircles=0;}
this._map.showHidePHGCircles();},getPHGCirclesState:function(){return this._map.state.showPHGCircles;},setPHGCirclesState:function(state){if(state>=0&&state<=2)
this._map.state.showPHGCircles=state;else
this._map.state.showPHGCircles=0;this._map.showHidePHGCircles();},toggleRNGCircles:function(){var state=this._map.state;if(state.showRNGCircles==0){state.showRNGCircles=1;}else if(state.showRNGCircles==1){state.showRNGCircles=2;}else{state.showRNGCircles=0;}
this._map.showHideRNGCircles();},getRNGCirclesState:function(){return this._map.state.showRNGCircles;},setRNGCirclesState:function(state){if(state>=0&&state<=2)
this._map.state.showRNGCircles=state;else
this._map.state.showRNGCircles=0;this._map.showHideRNGCircles();},toggleStationaryPositions:function(){if(this.isStationaryMarkersVisible()){this._map.state.isStationaryMarkersVisible=false;}else{this._map.state.isStationaryMarkersVisible=true;}
this._map.showHideMarkers();},getStationaryPositionsState:function(){return this._map.state.isStationaryMarkersVisible;},setStationaryPositionsState:function(state){this._map.state.isStationaryMarkersVisible=state;this._map.showHideMarkers();},toggleUnknownPositions:function(){if(this.isUnknownMarkersVisible()){this._map.state.isUnknownMarkersVisible=false;}else{this._map.state.isUnknownMarkersVisible=true;}
...
this._map.showHideMarkers();},getInternetPositionsState:function(){return this._map.state.isInternetMarkersVisible;},setInternetPositionsState:function(state){this._map.state.isInternetMarkersVisible=state;this._map.showHideMarkers();},toggleCwopPositions:function(){if(this.isCwopMarkersVisible()){this._map.state.isCwopMarkersVisible=false;}else{this._map.state.isCwopMarkersVisible=true;}
...
sizeStrValue+
scaleStrValue+
".svg";return trackdirect.settings.baseUrl+url;},_getIconFilePathCoursePart:function(course){let courseStrValue="";if(course!==null){let courseValue=Math.round(parseInt(course)/10)*10;while(courseValue>360){courseValue=courseValue-360;}
...
trackdirect.services.stationColorCalculator={_colors:["#E53935","#8E24AA","#3949AB","#00897B","#FB8C00","#6D4C41","#43A047","#F4511E","#D81B60",],_stationColorId:{},getColor:function(packet,trimmed=false){let colorId=this.getColorId(packet);if(trimmed===false)
return this._colors[colorId];return this._colors[colorId].replace("#","");},getKmlColor(packet){let hex=this.getColor(packet);let result=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);if(result){return"FF"+result[3]+result[2]+result[1];}
return null;},getColorId:function(packet){let colorId;if(packet.station_name in this._stationColorId){colorId=this._stationColorId[packet.station_name];return colorId;}else{let hash=this._simplehashStr(packet.station_name);colorId=Math.abs(hash)%this._colors.length;this._stationColorId[packet.station_name]=colorId;}
return colorId;},_simplehashStr:function(str){let hash=0;for(const char of str){hash=(hash<<5)-hash+char.charCodeAt(0);hash|=0;}
return hash;},};
...
if(this._heatmapCoordinates!==null&&this._heatmapCoordinates.length>0){this._heatmap=L.heatLayer(this._heatmapCoordinates,{minOpacity:0.75,radius:10,blur:4});}};trackdirect.models.StationCoveragePolygon.prototype._getConvexHullCoordinates=function(data,maxRange){let positions=this._getFilteredPositions(data,maxRange);positions.push(this._center);let xyPositions=this._convertToXYPos(positions);let convexHullXYPositions=convexhull.makeHull(xyPositions);let latLngPadding=this._paddingInPercentOfMaxRange*0.01*maxRange*0.000009;let latLngPaddingMin=this._paddingMinInMeters*0.000009;if(isNaN(latLngPadding)||latLngPadding<latLngPaddingMin){latLngPadding=latLngPaddingMin;}
...
dotMarker.hasLabel=false;dotMarker.showAsMarker=false;}};trackdirect.MarkerCreator.prototype._getDotMarkerIcon=function(packet){let colorId=trackdirect.services.stationColorCalculator.getColor(packet,true);let icon;if(typeof google==="object"&&typeof google.maps==="object"){icon={url:trackdirect.settings.baseUrl+
trackdirect.settings.imagesBaseDir+
"dotColor"+
colorId+
...
return false;};trackdirect.models.MarkerCollection.prototype.addStationCoverage=function(stationId,stationCoveragePolygon){if(stationCoveragePolygon!=null){this._stationCoverage[stationId]=stationCoveragePolygon;}else if(stationId in this._stationCoverage){delete this._stationCoverage[stationId];}};trackdirect.models.MarkerCollection.prototype.getStationCoverage=function(stationId){if(stationId in this._stationCoverage&&this._stationCoverage[stationId]!==null){return this._stationCoverage[stationId];}
return null;};trackdirect.models.MarkerCollection.prototype.getStationCoverageIds=function(){return Object.keys(this._stationCoverage).filter(key=>this._stationCoverage[key]!=null).map(Number);}
trackdirect.models.MarkerCollection.prototype.getStationIdListWithVisibleCoverage=function(){let result=[];for(let stationId in this._stationCoverage){if(this._stationCoverage[stationId].isRequestedToBeVisible()){result.push(stationId);}}
...
googleMap._initMap();return googleMap;}else if(typeof L==="object"){L.Map.call(this,document.getElementById(mapElementId),this._getLeafletMapOptions());this._updateLeafletTileLayer();L.control.zoom({position:"bottomright",}).addTo(this);this._initMap();}};if(typeof google==="object"&&typeof google.maps==="object"){trackdirect.models.Map.prototype=Object.create(google.maps.Map.prototype);}else if(typeof L==="object"){trackdirect.models.Map.prototype=Object.create(L.Map.prototype);}
...
if(lat<=90&&lat>=-90&&lng<=180&&lng>=-180){let content="";content+=this._getGpsDegreeFromGpsDecimal(lat,"lat");content+=" "+this._getGpsDegreeFromGpsDecimal(lng,"lon");content+=" | "+lat.toFixed(5)+", "+lng.toFixed(5);content+=" | ";let locatorLen=10;let zoom=this.getZoom();if(zoom<=4)locatorLen=2;else if(zoom<=6)locatorLen=4;else if(zoom<=10)locatorLen=6;else if(zoom<=13)locatorLen=8;let locator=this._getMaidenheadLocatorFromGpsDecimal(lat,lng,locatorLen);content+=locator;$("#"+options.coordinatesContainer).html(content);}};trackdirect.models.Map.prototype._getGpsDegreeFromGpsDecimal=function(dms,type){let sign=1,Abs=0;let days,minutes,secounds,direction;if(dms<0){sign=-1;}
...
100000).toFixed();days=days*sign;if(type=="lat")direction=days<0?"S":"N";if(type=="lon")direction=days<0?"W":"E";return(days*sign).toString().padStart(type=="lat"?2:3,"0")+"º "+minutes.toString().padStart(2,"0")+"' "+secounds.toString().padStart(2,"0")+"'' "+direction;};trackdirect.models.Map.prototype._getMaidenheadLocatorFromGpsDecimal=function(lat,lon,digits=6){if(lat<-90||lat>90||lon<-180||lon>180)return null;const A='ABCDEFGHIJKLMNOPQRSTUVWXYZ';const a='abcdefghijklmnopqrstuvwxyz';lon+=180;lat+=90;let fieldLon=Math.floor(lon/20);let fieldLat=Math.floor(lat/10);let squareLon=Math.floor((lon%20)/2);let squareLat=Math.floor((lat%10)/1);let subLon=Math.floor((((lon%2)/2)*24));let subLat=Math.floor((((lat%1)/1)*24));let remainLon=(((lon%2)/2)*24)-subLon;let remainLat=(((lat%1)/1)*24)-subLat;let extLon=Math.floor(remainLon*10);let extLat=Math.floor(remainLat*10);let extRemainLon=remainLon*10-extLon;let extRemainLat=remainLat*10-extLat;let ext2Lon=Math.floor(extRemainLon*24);let ext2Lat=Math.floor(extRemainLat*24);let locator="";if(digits>=2){locator+=A[fieldLon]+A[fieldLat];}
if(digits>=4){locator+=squareLon.toString()+squareLat.toString();}
if(digits>=6){locator+=A[subLon]+A[subLat];}
if(digits>=8){locator+=extLon.toString()+extLat.toString();}
if(digits>=10){locator+=a[ext2Lon]+a[ext2Lat];}
return locator;};
...
this._marker.packet.station_id+
', true); return false;');let coveragePolygon=this._defaultMap.markerCollection.getStationCoverage(this._marker.packet.station_id);if(coveragePolygon!==null&&coveragePolygon.isRequestedToBeVisible()){menuLink.html("Hide Coverage");}else{menuLink.html("Coverage");}
menuLi.append(menuLink);return menuLi;};trackdirect.models.InfoWindow.prototype.UpdateCoverageLink=function(status,isUserInput){let coverageLinkElementClass="stationCoverageLink"+this._marker.packet.station_id;if(status=="begin"){$("."+coverageLinkElementClass).html('Loading <i class="fa fa-spinner fa-spin" style="font-size:12px"></i>');}
if(status=="shown"){$("."+coverageLinkElementClass).html("Hide coverage");}
if(status=="hidden"){$("."+coverageLinkElementClass).html("Coverage");}
if(status=="not-enough-data"){$("."+coverageLinkElementClass).html("Coverage");if(isUserInput)alert("Currently we do not have enough data to create a max range coverage plot for this station. Try again later!");}
if(status=="load-failed"){$("."+coverageLinkElementClass).html("Coverage");if(isUserInput)alert("Coverage Data load failed");}}
trackdirect.models.InfoWindow.prototype._getMenuDivCenterLink=function(isInfoWindowOpen){let menuLi=$(document.createElement("li"));menuLi.css(this._getMenuDivLinkCss());let menuLink=$(document.createElement("a"));menuLink.css("color","#337ab7");if(isInfoWindowOpen){menuLink.attr("href","/center/"+
